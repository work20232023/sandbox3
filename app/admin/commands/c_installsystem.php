...
class InstallSystem extends CommandForm
{
	...
	function create()
	{
		list($hostname, $port) = \EnvironmentSettings::getDBHostPort($this->utf8towin($_REQUEST['MySQLHost']));
		...
		if ( !in_array('SkipStructure', array_keys($_REQUEST)) )
		{
			...
			if ( $_SERVER['WINDIR'] != '' )
			{
				...
				$mysql_command = 'call "'.$mysql_path.'" --host='.$hostname.' --port='.$port.' --user='.$username.
