...
class AdminPageMenu extends PageMenu
{
	...
	{
		$pages = [];
		$items = [['name' => text(2624), 'url' => '/admin/activity.php', 'uid' => 'acitivty']];
		if (!\EnvironmentSettings::getAutoUpdate()) {
			$items[] = ['name' => translate('Обновления'), 'url' => '/admin/updates.php', 'uid' => 'updates'];
		}

		$items = array_merge($items,			[
			['url' => '/admin/jobs.php', 'name' => text(2024), 'uid' => 'jobs'],
			['name' => translate('Резервные копии'), 'url' => '/admin/backups.php', 'uid' => 'backups'],
			['name' => translate('Логи'), 'url' => '/admin/log/', 'uid' => 'logs'],
			['name' => text(3109), 'url' => '/admin/webhooklogs/', 'uid' => 'webhooklogs']
		]);

		$pages[] = ['name' => '', 'items' => $items];
