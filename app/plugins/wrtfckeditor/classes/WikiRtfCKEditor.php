...
class WikiRtfCKEditor extends WikiEditorBase
{
	...
	function getTermsData(string $content): string
	{
		/** @var Term|null $term */
		...
		$term = getFactory()->getObject('Term');

		$objectIt = $term->search($content);

		$data = [];
		while (!$objectIt->end()) {
			$word = mb_strtolower($objectIt->get('Caption'));
			$data[] = [$word, $objectIt->getHtmlDecoded('Description')];
			$objectIt->moveNext();
		}
 	...
 	function draw( $content, $b_editable = false )
 	{
		...
		$toolbar = $this->getToolbar() == self::ToolbarMini ? "MiniToolbar" : "FullToolbar";
 		...
 		$attributes = array(
            ...
            'terms' => $this->getTermsData($content),
            'attributeName' => $field,
            'version' => $version,
            'userHeight' => $height,
