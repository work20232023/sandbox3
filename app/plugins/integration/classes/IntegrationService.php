...
class IntegrationService
{
    public function __construct( $object_it, $logger, $curlDelay = 0 )
    {
        ...
        $this->logFilePath = $this->setupLogger();
    }
    ...
    {
        $projectIt = getFactory()->getObject('Project')
                        ->getByRef('VPD', $this->object_it->get('VPD'));

        $logFilePath = SERVER_LOGS_PATH . '/' . \TextUtils::getFileSafeString(
            "integration-{$projectIt->get('CodeName')}-{$this->object_it->get('ProjectKey')}.log"
            );
