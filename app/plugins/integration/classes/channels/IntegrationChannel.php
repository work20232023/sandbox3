        ...
        foreach( $mapping as $attribute => $column )
        {
            ...
            if ( is_array($column) ) {
                ...
                if ( $column['type'] != '' )
                {
                    ...
                    if ( $id == '' && is_array($value) ) {
                        ...
                        foreach( $value as $valueItem ) {
                            ...
                            $remappedId = $this->idsMapRead[$column['type'].$valueId];
                            if ( $remappedId == '' ) {
                                $remappedId = $this->idsMapRead[$valueId];
                            }
                        ...
                        // process one-to-one reference or scalar
                ...
                if ( $column == '{parentId}' && $value != '' ) {
                    ...
                    $valueIds = [];
                    foreach( \TextUtils::parseIds($value) as $idValue ) {
                        $valueIds[] = $this->idsMapRead[get_class($object->getAttributeObject($attribute)).$idValue];
                    }
                    $value = array(
                        'Id' => join(',',$valueIds)
                    );
