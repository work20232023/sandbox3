...
class IntegrationJiraChannel extends IntegrationRestAPIChannel
{
    ...
    public function getItems( $timestamp, $limit )
    {
        ...
        do {
            $result = $this->jsonGet(self::apiPath . '/search', array(
                ...
                'maxResults' => $limit,
                'startAt' => $startAt,
                'fields' => 'updated,created,issuetype,comment,worklog,attachment,issuelinks,components'
            ));
            ...
            foreach ($result['issues'] as $issue) {
                if ($issue['fields']['issuetype']['subtask'] || in_array($issue['fields']['issuetype']['name'], $taskTypeMapping)) {
                    ...
                    $issueClassName = 'Request';

                    // scan for mapping and select appropriate entity for issue based on type mapping
                    foreach( $mapping as $entityClass => $entityMapping ) {
                        if ( end(explode('/', $entityMapping['url'])) != 'issue' ) continue;
                        foreach( $entityMapping as $entityAttribute => $attributeMapping ) {
                            if ( !is_array($attributeMapping) ) continue;
                            foreach( $attributeMapping as $mappingKey => $mappingValue ) {
                                if ( $mappingKey == 'mapping' ) {
                                    ...
                                    if ( in_array($issue['fields']['issuetype']['name'], $mappedTypes) ) {
                                        $issueClassName = $entityClass;
                                        break;
                                    }
                                }
                            }
        ...
        if ( $this->getObjectIt()->get('ProjectKey') != '' )
        {
            ...
            if ( is_array($mapping['Component']) ) {
                $result = $this->jsonGet(self::apiPath . "/project/{$this->getObjectIt()->get('ProjectKey')}/components",
                    array(), false);

                foreach ($result as $component) {
                    $components[] = array(
                        'class' => 'Component',
                        'id' => $component[$this->getKeyField()]
                    );
                }
            }
        }
        ...
        return array(
            array_merge(
                $users, $releases, $components, $first, $second, $latest
            ),
    ...
    public function mapFromInternal($class, $id, $source, $mapping, $setter)
    {
        ...
        foreach( $source as $attribute => $value ) {
            if ( $attribute == 'Capacity' ) {
                $source[$attribute] = round(floatval($source[$attribute]) * 60 * 60, 0);
            }
        }
    ...
    function buildDictionaries()
    {
        ...
        foreach( $this->jsonGet(self::apiPath."/issuetype?projectId={$projectData['id']}", array(), false) as $issueType )
        {
