...
function openPage( ids, force, progress_element, callback ) {
	...
	if (load_ids.length > 0 && load_ids[0] != ids[0] || load_ids.length < 1) {
		if (typeof callback == 'function') callback(ids[0]);
		callback = function (d) {};
	}
...
function mergeTitles( data )
{
	...
	$.each(sourceTable.find("tr[object-id]:not(.row-empty)"), function(index, value) {
		if ( $(value).find('input[name=treeTitle]').length > 0 ) {
...
function scrollToPage( page_id )
{
    ...
    var pos = item.offset().top - 125 - $('#tablePlaceholder .table-inner:first th[uid=content]').height();
