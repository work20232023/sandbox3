...
function checkRowsActions( group, action )
{
	switch (action) {
		case 'check_visible':
			$('#'+group).find('tbody tr td input[type=checkbox]').prop('checked', true);
			break;
		case 'all':
			$('#'+group+' tr th input[type=checkbox]').prop('checked', true);
			$('#'+group+' tbody tr td input[type=checkbox]').prop('checked', true);
			break;
		case 'uncheck':
			$('#'+group+' tr th input[type=checkbox]').prop('checked', false);
			$('#'+group+' tbody tr td input[type=checkbox]').prop('checked', false);
			break;
	}
                ...
                function (result, status, xhr) {
					...
					try {
						...
						if ( resultObject.message ) {
							...
							if ( resultObject.message == 'ok' && resultObject.description != '' ) {
								reportSuccess(resultObject.description);
							}
						}
					}
					catch (e) {
...
function appendEmbeddedItem( form_id, hidden )
{
	...
	var initializeFunctionName = 'initializeEmbedded' + form_id;
	if ( typeof window[initializeFunctionName] != 'undefined' ) {
		window[initializeFunctionName]();
	}
...
{
	var tabId = element.parents('.ui-tabs-panel').attr('id');
...
function saveEmbeddedItem( form_id, jfields, required, callback )
{
	...
	$('#embeddedFormBody'+form_id).ajaxSubmit({
		...
		success: function(data, status, xhr) {
			...
			$("#embeddedForm"+form_id+" [name*=LeftWork][default]").each(function() {
...
function checklistValidateOnForm(form)
{
	let input = null;

	form.find('div.ChecklistWrapper.required').find('input[type="checkbox"]').each(function() {
		if (!$(this).is(':checked')) {
			input = $(this);
			return false;
		}
	...
	if (null !== input) {
		formShowRequired(input.parent().parent().parent());
		return false;
	}
...
function validateForm( form )
{
	...
	if (valid) {
		valid = checklistValidateOnForm(form);
	}
...
function updateLeftWork( capacity, callback )
{
	window.setTimeout( function()
	{
		var capacityValue = convertUserHours(capacity.val());
		var left = capacity.closest('form').find('[name*=LeftWork]');

		if ( left.length > 0 && left.is('[default]') ) {
			var leftWork = convertUserHours(left.attr('default'));
			...
			left.val(leftValue);
			left.toggleClass('input-alert', leftWork - capacityValue < 0);
		}
...
function convertUserHours( value )
{
	var capacityValue = 0;
...
var asyncFormOptions =
{
	...
	reportError: function(formId, message)
	{
		$('#result'+formId)
			.addClass('alert alert-error').html(message);
	}
};
...
function makeAsyncForm( formId, url, message, options )
{
	...
	formOptions = {
		...
		error: function( xhr, status, error ) 
		{
			options.reportError(formId, ajaxErrorExplain( xhr, error ));
		...
		success: function( response, status, xhr ) 
		{
			...
			{
				options.reportError(formId, response);
                $('.btn[type=submit]').attr('disabled', false);
	 			return;
			}
            ...
            if ( state == 'redirect-error' ) {
				$('#result'+formId).removeClass('alert alert-success alert-error').html('');
                if ( message != '' ) {
					options.reportError(formId, message);
                }
					...
					buttons: [
						...
						},
						{
							...
							click: function()
							{
								...
								url.href = options.form_url;

								processBulk(
									text('form-delete'),
									url.pathname + '?formonly=true&operation=Method:BulkDeleteWebMethod:class=' + options.class_name,
									options.object_id,
									function() {
...
function getCheckedRows( ids )
{
    ...
    if ( allChecked.length > 0 && $('input[name*=to_delete_]:not(:checked)').length < 1 ) {
    	ids = $('table[items-hash]').attr("items-hash");
    	if ( ids != "" ) return ids;
	}
...
function moveNextCase()
{
	...
	try {
		var nextCase = parseInt($('.test-pagination a.btn-test-active').next().text());
		setupLocationParameter('offset1='+(nextCase-1));
	}
	catch(e) {
	}
}
