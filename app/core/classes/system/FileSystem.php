...
class FileSystem
{
    ...
    static function execPanDoc( $commands = '-v' )
    {
        ...
        if ( $streams['stderr'] != '' ) {
            throw new Exception('Unable execute command: ' . $streams['stderr']);
        }
    ...
    static function execLibreOffice( $commands = '--version' )
    {
        if ( \EnvironmentSettings::getWindows() ) {
            ...
            $cmd = "soffice -env:UserInstallation='file://".sys_get_temp_dir()."' " . $commands;
            $streams = self::execCommand($cmd);
        }
        if ( $streams['stderr'] != '' ) {
            throw new Exception("Unable execute command: {$cmd} " . PHP_EOL . $streams['stderr']);
        }
        return array_shift(explode(PHP_EOL, $streams['stdout']));
    }
