		...
		foreach( $attributes as $attribute )
		{
			...
			if ( !$validator->validate($parms[$attribute], $object->getAttributeGroups($attribute)) )
			{
				...
				if ( $result == '' ) {
					...
					if ( $typeName != '' ) {
                        $attributeGroups = $object->getAttributeGroups($attribute);
                        if ( in_array('hours', $attributeGroups) ) {
                            $typeName = text(3306);
                        }
                        if ( in_array('daily-hours', $attributeGroups) ) {
                            $typeName = sprintf(text(3305), defined('MAX_DAILY_HOURS') ? MAX_DAILY_HOURS : 24);
                        }
                        if ( in_array('non-negative', $attributeGroups) ) {
                            $typeName .= ', ' . text('validation.nonnegative');
                        }
						$result .= " ({$typeName})";
					}
