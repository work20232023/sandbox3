...
class Snapshot extends Metaobject
{
	...
	function freeze( $snapshot_id, $anchor, $items, $attributes )
	{
		...
		while ( !$anchor_it->end() )
		{
            if ( $anchor_it->get('Includes') > 0 ) {
                $valueObjectIt = $anchor_it->object->getExact($anchor_it->getId());
            }
            else {
                $valueObjectIt = $anchor_it->copy();
            }
			...
			foreach ( $attributes as $attribute )
			{
			    ...
			    switch ( $type ) {
                    case 'wysiwyg':
                        ...
                        break;
                    default:
                        $value = $valueObjectIt->getHtmlDecoded($attribute);
                        break;
                }
