...
class IteratorExportExcel extends IteratorExport
{
    ...
    function worksheet()
    {
        ...
        while( !$it->end() ) {
            if ($group) {
                $groupValue = $this->getValue( $group, $it )[0];
                if (!in_array($groupValue, $groupValues))
                {
                    if ( $it->object->IsReference($group) ) {
                        $attributeObject = $it->object->getAttributeObject( $group );
                        $groupView = $attributeObject->getDisplayName() . ': ';
                    }
                    else {
                        $groupView = $it->object->getAttributeUserName($group) . ': ';
                    }

                    if (strlen($groupValue) > 0) {
                        $groupView .= $groupValue;
                    } else {
                        $groupView .= translate('нет');
                    }

                    $sheet->setCellValueByColumnAndRow(1, $row, $groupView)->getStyle('A' . $row . ':Z' . $row)->getFill()
 	...
 	function getValue( $key, $iterator )
 	{
        ...
        switch ( strtolower($type) )
        {
            ...
            case 'wysiwyg':
                ...
                break;
            default:
                $groups = $iterator->object->getAttributeGroups($key);
                if ( in_array('hierarchy-parent', $groups) ) {
                    $value = $iterator->getRef($key)->getDisplayName();
                    return array( $value, Cell\DataType::TYPE_STRING );
                }
