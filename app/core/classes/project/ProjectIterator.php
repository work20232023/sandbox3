...
class ProjectIterator extends OrderedIterator
{
    ...
    function getLeadIds()
    {
        return $this->object->createSQLIterator(
                    "SELECT p.pm_ParticipantId FROM pm_Participant p 
                       WHERE EXISTS (SELECT 1 FROM pm_ParticipantRole r, pm_ProjectRole l, pm_ProjectRole rb 
                                      WHERE r.Participant = p.pm_ParticipantId 
                                        AND r.ProjectRole = l.pm_ProjectRoleId
                                        AND l.ProjectRoleBase = rb.pm_ProjectRoleId
                                        AND rb.ReferenceName = 'lead' ) 
                            AND p.Project = {$this->getId()}
                            AND NOT EXISTS (SELECT 1 FROM cms_BlackList bl WHERE bl.SystemUser = p.SystemUser) "
                )->fieldToArray('pm_ParticipantId');
    }
