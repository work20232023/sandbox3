...
class TextUtils
{
    ...
    public static function stripAnyTags( $text ) {
        $text = preg_replace(
            '/(?:<|&lt;)\/?(a|p|div|br|h[\d]+|span|ul|ol|li|strong|font|br|hr|table|tr|td|th|tbody|colgroup|col|thead|button|iframe|html|body|link|script|style|i|b|del|strike|em|code|pre|title|\!\-\-)\s*[^>]*?(?:>|&gt;)/i',
    ...
    public static function getCleansedHtml( $body )
   {
    ...
    public static function getUnstyledHtml( $content )
    {
        $content = preg_replace_callback(
            '#<[a-z]+\s+[^>]+>#',
            function($match) {
                return str_replace('style="', 'was-style="',
                            str_replace('class="', 'was-class="', $match[0]));
            }, $content);
    ...
    public static function parseIds( $text )
    {
        ...
        if ( is_numeric($text) ) {
            return $text > 0 ? array($text) : array();
        }

        if ( preg_match('/^[0-9\,\-\s]+$/', $text) === 0 ) {
