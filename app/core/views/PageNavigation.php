...
class PageNavigation
{
    ...
    {
        $size = 30;
        list($row, $column) = \EnvironmentSettings::getSpritePosition($size, $user_it->getId());
        $timestamp = getSession()->getUserPicTimestamp();

        return array (
            'class' => 'header_popup',
            'title' => '<span class="profile-avatar" style="background: url(\'/images/userpics-middle.png?v='.$timestamp.'\') no-repeat '.(-1 * $column * $size).'px '. (-1 * $row * $size).'px;" title="'.text(2637).'"></span>',
            'items' => $actions,
        );
    }
