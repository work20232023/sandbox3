...
class BulkFormBase extends AjaxForm
{
    ...
    function __construct( $object, $entityFormClass )
    {
    ...
    {
        if ( $this->entityFormClass == "" ) return null;
        if ( !class_exists($this->entityFormClass) ) return null;

        $class_name = $this->entityFormClass;
        return new $class_name($this->getObject());
    }
	...
	function getIt()
	{
	    ...
	    $ids = array_unique(\TextUtils::parseIds(urldecode($_REQUEST['ids'])));
	    if ( count($ids) < 1 ) {
    ...
    {
        $matches = [];
        if ( preg_match('/baseline=([^:=]+)/', $_REQUEST['operation'], $matches) ) {
	...
	{
		$uid = new ObjectUID($this->getBaseline());
