...
class PageForm extends Form
{
	...
	function createFieldObject( $name )
	{
		...
		if( $this->object->IsReference( $name ) )
		{
            ...
            if ( is_object($this->getObjectIt()) && !$this->getObject()->getAttributeEditable($name) )
            {
                return new FieldListOfReferences(
                    $this->getObjectIt()->getRef($name)
                );
            }
            else {
    ...
    function getActions()
	{
		...
		{
			$transition_actions = $this->getTransitionActions($this->getObjectIt());
 	...
 	function getTransitionActions($object_it)
	{
        ...
        if ( $object_it->getId() == '' ) return $actions;
        ...
        if ( count($actions) > 0 ) {
            // display reason only when there are no available transitions
            $this->transition_messages = [];
        }
	...
	function getRenderParms()
	{
		...
		if ( is_object($object_it) )
		{
			if ( $uid->hasUid( $object_it ) )
			{
			    ...
			    $uid_number = "{$info['project']}:{$info['uid']}";
			    $uid_url = $info['url'];
			}
			else
			{
	...
	protected function getSourceParms($attributes, $baseline = '')
	{
        ...
        foreach( $this->getSourceIt() as $item ) {
            ...
            if ( is_subclass_of($class_name, 'IteratorExport') ) {
                ...
                $iteratorObject->setOptions(['numbering', 'uid']);
            ...
            if ( $source_it->getId() != '' ) {
                $parms['source' . $class_name . $source_it->getId()] = array(
                    'sectionName' => translate('Источники'),
                    'sectionParms' => array (
                        'html' => '<div class="source-text-header">
                                       '.$uid->getUidWithCaption($source_it).'
                                   </div>
