...
class PageChart extends StaticPageList
{
    ...
    function buildIterator()
	{
	    ...
	    if ( $this->getGroup() == 'history' )
		{
		    ...
		    $it = $object->getAggregatedHistory(
                $aggs,
                array_merge($predicates, [new FilterModifiedAfterPredicate($values['modifiedafter'])])
            );
		}
		elseif ( count($aggs) > 0 )
		{
			$it = $object->getAggregated($aggs, $predicates);
		}
		else {
	...
	function getChartSettings()
    {
        ...
        if ( count($fields) > 0 ) {
            $columns = array(
                text(2536) => array(
                    'click' => "javascript: filterLocation.setup( 'aggby=all', 1 ); ",
                    'checked' => $filter_values['aggby'] == 'all'
                )
            );

            foreach ( $fields as $field ) {
                $columns[translate($object->getAttributeUserName( $field ))] = array(
                    'click' => "javascript: filterLocation.setup( 'aggby={$field}', 1 ); ",
                    'checked' => $filter_values['aggby'] == $field );
            }
            ksort($columns);
        }
