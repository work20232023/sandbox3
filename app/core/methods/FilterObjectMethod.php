...
class FilterObjectMethod extends FilterWebMethod
{
	...
	function getValues()
	{
		...
		return array_merge($values, $this->getValuesText($selected_values));
	}
    ...
    function getValuesText( $values )
    {
        ...
        if ( count($values) < 1 ) return $items;
        ...
        $object_it = $registry->Query(
            array (
                $this->object->getIdAttribute() == $this->idfieldname
                    ? new FilterInPredicate($values)
                    : new FilterAttributePredicate($this->idfieldname, $values),
                new FilterVpdPredicate()
            )
        );
        while ( !$object_it->end() ) {
            $items[' '.$object_it->get($this->idfieldname)] = $uid->getUidTitle($object_it);
            $object_it->moveNext();
        }
    ...
    function parseFilterValue($value, $context)
    {
        ...
        $value = preg_replace_callback('/(user\-tasks)/i', function($match) use($context) {
            ...
            $it = $context->getAggregated([$aggregate]);
