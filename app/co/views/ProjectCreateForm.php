	 ...
	 function drawCustomAttribute( $attribute, $value, $tab_index, $view )
	 {
		 switch ( $attribute )
		 {
             ...
             case 'Users':
                 ...
                 $isGroup = true;
                 $list = true;
                 $groupIt = getFactory()->getObject('UserGroup')->getAll();
                 while( !$groupIt->end() ) {
                     if ($groupIt->getUsers() < 1) {
                         $groupIt->moveNext();
                         continue;
                     }
                     $groupsData[$groupIt->getId()] = [
                         'name' => $groupIt->getDisplayName(),
                     ];
                     $groupIt->moveNext();
                 }

                 if (count($groupsData) < 1) {
                     $isGroup = false;
                 }
                 ...
                 while( !$userIt->end() ) {
                     ...
                     if ($isGroup) {
                         ...
                         if ($userIt->get('GroupId')) {
                             $groupIds = explode(',', $userIt->get('GroupId'));
                             foreach ($groupIds as $groupId) {
                                 $groupsData[$groupId]['items'][] = [
                                     'id' => $userIt->getId(),
                 ...
                 if (array_key_exists('other', $groupsData)) {
                     $groupsData['other']['name'] = text(3513);
                 }

                 if ($isGroup && count($groupsData) == 1) {
                     $groupsData = array_values($groupsData)[0]['items'];
                     $list = true;
                 }
