...
class IssueController extends Controller
{
    ...
    public function showAction($id)
    {
        ...
        $service->clearNotifications($issue, $this->getUser());

        return $this->render('Issue/show.html.twig', array(
            'issue' => $issue,
            'comment_hierarchy' => $service->getCommentHierarchy($issue->getComments()),
            'comment_form' => $commentForm->createView(),
            'IsVisibleSecondBtnApplication' => count($issue->getComments()) > 0
        ));
    }
    ...
    public function editAction($id)
    {
        ...
        if ($issue->getState()->getNotChange() == 'Y') {
            return $this->redirect($this->generateUrl('issue_list'));
        }
    ...
    public function productAction()
    {
        ...
        return $this->render('Issue/product.html.twig', array(
            'projects' => $this->getIssueService()->getProducts($this->getProjectVpds())
        ));
    }
    ...
    /**
     * @Route("/search", name="issue_search")
     * @Method("GET")
     * @Template()
     */
    public function searchAction( Request $request )
    {
        ...
        $service = $this->getIssueService();
        $issues = $service->getSearchIssues($this->getProjectVpds(), $this->getUser()->getEmail(), $searchQuery);

        if ( count($issues) < 1 ) {
            return $this->redirect($this->generateUrl('issue_list'));
        }
        if ( count($issues) == 1 ) {
            return $this->redirect($this->generateUrl('issue_show', array('id' => $issues[0]->getId())));
        }
    ...
    /**
     * @Route("/issue/{id}/{target}/state", name="issue_edit_state")
     * @Method("GET")
     */
    public function editStateAction($id, $target)
    {
        if (!is_object($this->getUser())) throw new HttpException(403);
        ...
        $this->getIssueService()->updateState($issue, $this->getUser(), $target);
