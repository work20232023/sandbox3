...
class IssueService {
    ...
    public function updateIssue(Issue $issue, User $author, array $post)
    {
        ...
        $changed = $this->objectChangeLogger->getChangedFields($issue);
        $wasObjectIt = $this->objectChangeLogger->getIssueToBeModified($issue, $author);

        $this->em->persist($issue);
        $this->em->flush();

        $this->updateCustomAttributes($issue, $author, $post);
        $this->objectChangeLogger->notifyIssueModified($issue, $author, $wasObjectIt, $changed);
    }
    ...
    /** @return Issue[] */
    public function getSearchIssues(array $vpds, $email, $searchQuery) {
        $issueRepository = $this->em->getRepository('DevpromServiceDeskBundle:Issue');
        return $issueRepository->createQueryBuilder('o')
            ->leftJoin('o.author', 'author')
            ->leftJoin('o.customer', 'customer')
            ->where('(o.vpd IN (:vpds))')
            ->andWhere('(o.caption LIKE :query or o.description LIKE :query)')
            ->andWhere('(author.email = :email or customer.email = :email)')
    ...
    public function updateState(Issue $issue, User $author, $target)
    {
        $this->buildProjectSession($issue, $author);

        $class_name = getFactory()->getClass($issue->getState()->getObjectClass());
        $object = getFactory()->getObject($class_name);
        $object_it = $object->getExact($issue->getId());

        $target_state = getFactory()->getObject('IssueState')->getExact($target)->get('ReferenceName');

        $service = new WorkflowService($object_it->object);
        $service->moveToState( $object_it, $target_state);

        getFactory()->resetCachedIterator($object);
    }

    public function getProducts( array $vpds )
    {
        $projects = $this->em->getRepository('DevpromServiceDeskBundle:Project')
                        ->findBy(
                            array('vpd' => $vpds, 'closed' => 'N'),
                            array('index' => 'ASC', 'importance' => 'ASC', 'name' => 'ASC')
                        );
     ...
     */
    public function getCommentHierarchy(array $comments): array
    {
        $hierarchy = [];

        // Create node for each comment
        ...
        $commmentWithParentIds = [];
        foreach ($comments as $article) {
            $parent = $article->getParentComment();
            if (!isset($parent)) {
                continue;
            }
            ...
            $id = $article->getId();
            $hierarchy[$parentId]['children'][] = &$hierarchy[$id];
            $commmentWithParentIds[] = $id;
        }

        // Remove parented nodes at the top level
        foreach ($commmentWithParentIds as $id) {
            unset($hierarchy[$id]);
        }
