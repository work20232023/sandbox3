...
class ObjectChangeLogger
{
     ...
     */
    public function getIssueToBeModified(Issue $issue, User $user)
    {
        $this->buildProjectSession($issue, $user);
        return getFactory()->getObject('Request')->getExact($issue->getId());
    }
     ...
     * @return array
     */
    public function getChangedFields(Issue $issue)
    {
    ...
    public function notifyIssueModified(Issue $issue, User $user, $stateIt, array $changed)
    {
        ...
        getFactory()->getEventsManager()->notify_object_modify(
            $stateIt->copy(), $object_it->copy(), array_flip(array_keys($changed)));
    }
