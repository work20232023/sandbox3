...
class TooltipService
{
    ...
    protected function buildAttributes( $object_it )
    {
 		...
 		foreach ( $object->getAttributes() as $attribute => $parms )
 	 	{
            ...
            $data[] = array (
                ...
                'modelGroups' => $object->getAttributeGroups($attribute)
 	 		); 
 	 	}
 	...
 	{
        $attributeGroups = $object_it->object->getAttributeGroups($attribute);
 		...
 		switch ( $type )
 		{
            ...
            case 'integer':
                $value = $object_it->get($attribute);
                if ( in_array('percentage', $attributeGroups) ) {
                    $value .= '%';
                }
                return $value;
