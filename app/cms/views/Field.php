...
class Field
{
    ...
    {
		$paramsArray = [self::class, "iterator_url_callback"];

        $text = preg_replace('/&quot;/', '"', $text);
        $text = preg_replace_callback('/\[url=([^\]]+)\s+text=([^\]]+)]/im', $paramsArray, $text);
        $text = preg_replace_callback('/\[url=([^\]]+)]/im', $paramsArray, $text);

        $url = '~(?:(https?)://([^<^\s]+)|(www\.[^\s<]+?\.[^\s<]+))(?<![\.,:])~i';
        $text = preg_replace($url, '<a href="$0" target="_blank">$0</a>', $text);

        $text = preg_replace_callback('/(^|[^\w\.\,\:\;\/\#">]+)(\[?[A-Z]{1}-[0-9]+\]?)([\s]*|$)/mi', [self::class, "iterator_uid_callback"], $text);
