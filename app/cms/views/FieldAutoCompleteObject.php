    ...
    function buildObjectIt()
 	{
		...
		$registry = $this->getObject()->getRegistryBase();
	...
	function getText()
	{
 	...
 	function draw( $view = null )
 	{
		...
		else
		{
            ...
            if ( $displayValue == '' && $this->getAppendable() ) {
                $displayValue = $this->getDefault();
            }

            $value = $object_it->getId();
            if ( $value == '' && $this->getAppendable() ) {
                $value = $this->getDefault();
            }

            echo '<input type="text" class="autocomplete-text input-block-level" for-input="'.$this->getName().'" id="'.$this->getName().'Text" auto-expand="'.($this->auto_expand?'true':'false').'" tabindex="'.$this->getTabIndex().'" style="'.$this->getStyle().'" placeholder="'.text(1338).'" default="'.$displayValue.'" value="'.$displayValue.'" '.($this->getRequired() ? 'required' : '').' multiple="'.var_export($this->multiple, true).'" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">';
            echo '<input class="fieldautocompleteobject" type="hidden" name="'.$this->getName().'" id="'.$this->getId().'" default="'.$this->getDefault().'" value="'.$value.'" object="'.get_class($object).'" caption="'.$text.'" searchattrs="'.join(',', $this->getAttributes()).'" additional="'.join(',', $this->getAdditionalAttributes()).'" '.($this->getRequired() ? 'required' : '').' ondblclick="javascript: '.$this->getOnSelectCallback().';" project="'.$project.'">';
		}
 	}
