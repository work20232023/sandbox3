...
class FieldHours extends Field
{
	...
	{
		if ( $this->readOnly() ) {
            echo '<input class="input-medium" type="text" id="'.$this->getId().'" name="'.$this->getName().'Text" value="'.$this->getValue().'" default="'.$this->getDefault().'" readonly>';
            echo '<input type="hidden" name="'.$this->getName().'" value="'.parent::getValue().'">';
		}
		else {
