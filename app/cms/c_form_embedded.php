...
class FormEmbedded
{
	...
	function drawField( $attr, $type, $value, $tabindex )
	{
		...
		else
		{
            $field = $this->createField( $attr );
            if ( is_object($field) ) {
                $fieldValue = $this->getFieldValue($attr);
                $field->setName( $field_name );
                $field->setId( $field_name );
                $field->setValue( $value );
                $field->setTabIndex( $tabindex );
                $field->setDefault( $fieldValue != '' ? $fieldValue : $this->object->getDefaultAttributeValue($attr) );
 	...
 	function process( $object_it, $e, $process_record_callback = null )
 	{
        ...
        foreach( $_REQUEST as $key => $value )
        {
            ...
            foreach ( $fields as $field )
            {
                ...
                {
                    $tmp_file = getFactory()->getObject('cms_TempFile');
