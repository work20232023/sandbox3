...
class ObjectRegistrySQL extends ObjectRegistry
{
    ...
    public function QueryById( $ids )
    {
        ...
        $sortClause = $this->getSortClause(array(), 't');
        if ( $sortClause != '' ) $sortClause = ' ORDER BY '.$sortClause;

        return $this->createSQLIterator(
            "SELECT {$this->getSelectClause($this->extractPersisters(array()), 't')} 
                  FROM {$this->getQueryClause(array())} t 
                 WHERE 1 = 1 {$filter->getPredicate()} {$sortClause} "
        );
    }
	...
	public function Query( $parms = array() )
	{
        ...
        if ( $this->wrapSQL ) {
            ...
            $filterClause = $this->getFilterClause($parms, $alias);
            if ( $filterClause != "" ) {
                $filterClause = " WHERE 1 = 1 {$filterClause} ";
            }

            $sql = "SELECT t.* FROM ( SELECT {$fieldsClause}, {$selectClause} 
                      FROM {$this->getQueryClause($parms)} {$alias} {$filterClause} ) t";
        }
        else {
    ...
    public function getFilterClause( $parms, $alias )
    {
        ...
        return $this->getFilterPredicate($filters, $alias);
    }
    ...
    public function QueryKeys( $parms = array(), $do_exec = true )
    {
        ...
        $sorts = $this->extractSorts($parms);
        $persisters = array_filter(
            $this->extractPersisters($parms),
            function($persister) use ($sorts) {
                foreach( $sorts as $sort ) {
                    if ( !$sort instanceof \SortAttributeClause ) continue;
                    if ( !in_array($sort->getAttributeName(), $persister->getAttributes()) ) continue;
                    return true;
                }
                return false;
            }
        ...
        $selectClause = $this->getSelectClause($persisters, 't', false);
        if ( $selectClause != '' ) $selectClause = ", " . $selectClause;

        $sql = "SELECT t.{$this->getObject()->getIdAttribute()} {$selectClause}
                  FROM {$this->getQueryClause($parms)} t
                 WHERE 1 = 1 {$this->getFilterPredicate($filters)} ";

        $sort = $this->getSortClause($sorts, 't');
        if ( $sort != '' ) $sql .= ' ORDER BY '.$sort;
    ...
    public function Merge( array $data, array $alternativeKey = array() )
    {
        ...
        if ( $data[$this->getObject()->getIdAttribute()] != '' ) {
            $parms[] = new FilterInPredicate($data[$this->getObject()->getIdAttribute()]);
        }
        else {
