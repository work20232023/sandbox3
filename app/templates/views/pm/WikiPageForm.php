...
<h4 class="title-cell bs" style="padding-left:0;padding-right: 3px;">
	<div class="sec-num">
...
<?php } ?>
    ...
    <?php if ( $persisted && $uid_icon != '' && $attributes['UID']['visible'] ) { ?>
        <div class="title-cell">
            <? echo $view->render('core/ClipboardButton.php', array ('url' => $uid_url, 'uid' => $uid)); ?>
        </div>
    <?php } ?>

<? if ( array_key_exists('Caption', $attributes ) && array_key_exists('field', $attributes['Caption']) ) : ?>
...
<?php } ?>
	...
	<?php if ( $persisted && $baseline == '' && is_a($form->getObjectIt(), 'StatableIterator') && $attributes['State']['value'] != '' && $attributes['State']['visible'] ) { ?>
    ...
    <?php
        ...
        echo $view->render('pm/StateColumn.php', array (
            'stateIt' => $form->getSectionIt()->getStateIt(),
