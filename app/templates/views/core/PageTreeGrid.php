...
<div class="list-container">
    <table id="<?=$table_id?>" class="table-inner <?=$table_class_name?>" created="<?=$created_datetime?>" uid="<?=$widget_id?>" items-hash="<?=$itemsHash?>">
		...
		<tr class="header-row">
			...
			<th class="for-chk visible" uid="checkbox" width="30">
                <div class="btn-group">
...
<script language="javascript">
    $(function(){
        ...
        $("#<?=$table_id?>").fancytree({
            ...
            renderColumns: function(event, data) {
                ...
                $(data.node.tr)
                    ...
                    .attr('object-id', data.node.data['self-id'])
                    .attr('modified', data.node.data['modified']);

                $(data.node.tr).find(">td input").change(function(e){
        ...
        $("#<?=$table_id?>").on("click", "", function(e){
            ...
            if ( node && node.tr && $(e.target).is('td,.fancytree-title') ) {
                if ( node.data['self-id'] ) {
                    $(document).trigger("trackerItemSelected",
                        [node.data['self-id'], e.ctrlKey || e.metaKey || $(e.target).is('input'),
                            node.data['class']]);
                }
            }
        });
