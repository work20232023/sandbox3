...
$showRightColumn = count($source_parms) > 0;
?>
<div class="tabs">
    ...
    <?php } ?>
	    <form class="form-horizontal <?=$form_class?>" id="<?=$form_id?>" method="post" action="<?=$form_processor_url?>" enctype="<?=($formonly ? "application/x-www-form-urlencoded" : "multipart/form-data")?>" autocomplete="off" class_name="<?=$form_class_name?>">
	    	<fieldset>
				...
				<div id="tab-main">
					<div class="<?=($showRightColumn ? 'source-left' : '')?>">
					<?php
					...
					<? if ($showRightColumn) { ?>
						<div class="source-text">
                            ...
                            <? foreach( $source_parms as $key => $parameter) { ?>
                                <div class="accordion-heading">
                                    <a class="to-drop-btn" href="#<?=$key?>" data-toggle="collapse" tabindex="-1">
                                        <span class="caret"></span>
                                        <?=$parameter['sectionName']?>
                                    </a>
                                </div>
                                <div id="<?=$key?>" class="accordion-body in collapse">
                                    <?=$parameter['sectionParms']['html']?>
                                </div>
                            <? } ?>
							</div>
						</div>
