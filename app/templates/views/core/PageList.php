...
<? } ?>
        ...
        <table id="<?=$table_id?>" class="table-inner <?=$table_class_name?>" created="<?=$created_datetime?>" uid="<?=$widget_id?>" items-hash="<?=$itemsHash?>">
		...
		<tr class="header-row">
			...
			<th class="for-chk <?=($need_to_select ? 'visible' : 'hidden')?>" uid="checkbox" width="30">
				<?php if ( $need_to_select ) { ?>
                    <div class="btn-group">
                        <a class="dropdown-toggle transparent-btn list-action-btn" uid="widget-more-list-actions" href="" data-toggle="dropdown">
                            <input id="to_delete_all<?=$table_id?>" tabindex="-1" type="checkbox" class = "to_delete_all_check" disabled>
                            <span class="caret"></span>
                        </a>
                        <? echo $view->render('core/ListActionsMenu.php', array('table_id' => $table_id, 'uid' => 'widget-more-list-actions')); ?>
                    </div>
				<?php } ?>
			</th>
					...
					foreach( $columns as $attr )
					{
                        ...
                        if( $columns_info[$attr]['reference'] ) {
                            ...
                            foreach( $plugins_interceptors as $plugin ) {
                                if ( $plugin->interceptMethodListDrawCell( $list, $it, $attr ) ) {
						...
						if ( count($actions) == 1 )
						{
							...
							if ( $action['url'] != '' ) {
                                ...
                                <div class="btn-group visible-operations last">
                                    <a id="<?=$action['uid']?>" class="btn btn-info btn-xs dropdown-toggle actions-button" href="<?=$action['url']?>"><?=$action['name']?></a>
                                </div>
                                <?
                            } else if ( $action['click'] != '' ) {
                                ?>
                                <div class="btn-group visible-operations last">
                                    <a id="<?=$action['uid']?>" class="btn btn-info btn-xs dropdown-toggle actions-button" onclick="<?=$action['click']?>"><?=$action['name']?></a>
                                </div>
                                <?
                            }
