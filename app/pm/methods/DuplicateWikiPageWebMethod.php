 	...
 	function duplicate( $project_it, $parms )
 	{
 		...
 		else
 		{
			...
			if ( $this->getObjectIt()->get('ParentPage') == '' )
			{
                ...
                while( !$objectIt->end() )
                {
                    $documentId = $map[$this->getObject()->getEntityRefName()][$objectIt->getId()];
                    ...
                    if ( $_REQUEST['Snapshot'] != '' ) {
                        $version_it = getFactory()->getObject('Snapshot')->getExact($_REQUEST['Snapshot']);
                        $sourceBaselineName = $version_it->getDisplayName();
                    }
                    if ( $sourceBaselineName == "" ) {
                        $sourceBaselineName = $objectIt->get('DocumentVersion');
                    }

                    if ( $sourceBaselineName != '' ) {
                        $baselineService->storeBaseline($documentIt, $sourceBaselineName);
                    }
                    else {
