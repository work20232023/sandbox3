...
class DuplicateWebMethod extends WebMethod
{
    ...
    function getAttributesDefaults( $iterator, $attributes = array() )
    {
        ...
        if ( count($attributes) < 1 ) {
            $attributes = $iterator->object->getAttributesStored();
        }
        foreach( $attributes as $attribute ) {
            $override[$attribute] = $iterator->getHtmlDecoded($attribute);
        }
	    return $override;
    }
        ...
        $wasProjectIt = getSession()->getProjectIt();
 	    ...
 	    foreach( $references as $object )
 	    {
 	        ...
 	        if ( get_class($object_it->object) == get_class($object) )
 	        {
 	            $rowset = array();
 	            while( !$iterator->end() ) {
                    $data = $this->getAttributesDefaults($iterator);
                    $data[$object->getIdAttribute()] = $iterator->getId();
                    $rowset[] = $data;
                    $iterator->moveNext();
                }
