...
class ReintegrateWikiTraceWebMethod extends WebMethod
{
	...
	function execute_request()
 	{
        ...
        if ( $_REQUEST['Branch'] != '' ) {
            $documentIt = $targetIt->getRef('DocumentId');
            if ( !\TextUtils::isNullValue($documentIt->get('UID')) ) {
                $documentIt = $object->getRegistry()->Query(
                    array(
                        new WikiPageBranchFilter($_REQUEST['Branch']),
                        new FilterTextExactPredicate('UID', $documentIt->get('UID'))
                    )
                );
            }
            else {
        ...
        while( !$targetIt->end() )
        {
            if ( !\TextUtils::isNullValue($targetIt->get('UID')) ) {
                ...
                $sourceIt = $object->getEmptyIterator();
            }

            if ( $sourceIt->getId() == '' && $targetIt->get('ParentPage') != '' )
            {
                $pageIt = $targetIt->getRef('ParentPage');
                if ( !\TextUtils::isNullValue($pageIt->get('UID')) ) {
                    $sourceIt = $object->getRegistry()->Query(
                        array(
                            new WikiDocumentFilter($documentIt),
                            new FilterTextExactPredicate('UID', $pageIt->get('UID'))
                        )
                    );
                }
                else {
    ...
    function hasAccess()
    {
        return getFactory()->getAccessPolicy()->can_modify($this->toIt);
    }
}