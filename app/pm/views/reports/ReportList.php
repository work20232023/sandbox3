...
class ReportList extends PMPageList
{
	...
	function drawGroup( $group_field, $object_it )
	{
		switch ( $group_field ) {
			case 'Category':
			    $category_it = getFactory()->getObject('PMReportCategory')->getRegistry()->Query([
                    new FilterAttributePredicate('ReferenceName', $object_it->get('Category'))
                ]);

			    if ( $category_it->getId() ) {
				    echo "<b>{$category_it->getDisplayName()}</b>";
			    }
			    else {
