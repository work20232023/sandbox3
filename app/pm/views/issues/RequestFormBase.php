...
class RequestFormBase extends PMPageForm
{
	...
	function __construct( $object )
	{
        ...
        $this->featureIt = $_REQUEST['FromFeature'] > 0
            ? getFactory()->getObject('Feature')->getExact($_REQUEST['FromFeature'])
            : getFactory()->getObject('Feature')->getEmptyIterator();
	}
    		...
    		default:
    			...
    			if ( $_REQUEST['Requirement'] > 0 && $attr == 'Description' ) {
                    if ( $_REQUEST['Baseline'] > 0 ) {
                        $parms = $this->getSourceParms([], $_REQUEST['Baseline']);
                        return $parms[0]['text'];
                    }
                    else {
                ...
                if ( $this->featureIt->getId() > 0 ) {
                    return $this->featureIt->getHtmlDecoded($attr);
                }
    ...
    protected function getSourceParms($attributes, $baseline = '')
    {
        ...
        if ( getSession()->getProjectIt()->getMethodologyIt()->get('IsSupportUsed') === 'Y'
                && $this->getTransitionIt()->getId() !== null )
        {
            $parms['similarProjectPage'] = [
                'sectionName' => text(3703),
                'sectionParms' => [
                    'html' => '<script type="text/javascript">
                                    $(function(){replaceAjaxHtml("#similarProjectPage", "analysis/ProjectPage/Request/'.$this->getObjectIt()->getId().'");});
                                   </script>'
                ]
            ];

            $parms['similarHelpPage'] = [
                'sectionName' => text(3704),
                'sectionParms' => [
                    'html' => '<script type="text/javascript">
                                    $(function(){replaceAjaxHtml("#similarHelpPage", "analysis/HelpPage/Request/'.$this->getObjectIt()->getId().'");});
                                   </script>'
                ]
            ];
        }
        ...
        return array_merge(
            $parms,
            parent::getSourceParms($attributes, $_REQUEST['Baseline'])
        );
    }
