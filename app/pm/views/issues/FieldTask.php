...
class FieldTask extends Field
{
 	...
 	function draw( $view = null )
 	{
		...
		if ( $taskboxes < 1 ) {
            $this->drawForm( $task->createCachedIterator(array($parms)), '', $parms );
            $taskboxes++;
        }
		...
		for ( $i = $taskboxes; $i < 8; $i++ ) {
			$this->drawForm( $task->createCachedIterator(array($parms)), 'display:none;', $parms );
			$taskboxes++;
		}
 	...
 	function drawByTypes( $task, & $taskboxes, $skipTypes )
 	{
		...
		$tasktype_it = getFactory()->getObject('pm_TaskType')->getRegistry()->Query($filters);
 		...
 		while ( !$tasktype_it->end() )
 		{
			...
			$this->drawForm( $task->createCachedIterator(array($parms)), 'display:block;');
 	...
 	function drawForm( $ref, $style = '', $closeBoxButton = true )
 	{
	 	$form = new FormTaskEmbedded( $ref->object,'ChangeRequest', '' );

 		$form->setSingleton( true );
 		$form->setReadonly( false );
 		$form->setObjectIt($ref);
