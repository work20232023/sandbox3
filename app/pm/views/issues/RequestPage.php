...
class RequestPage extends PMPage
{
	function __construct()
	{
        ...
        if ( in_array($_REQUEST['view'], ['board']) ) {
            getSession()->addBuilder( new CommentStatModelBuilder(['pm_ChangeRequest']) );
        }
            ...
            if ( $_REQUEST['mode'] == 'group' ) {
                ...
                if (is_object($object_it) && $object_it->getId() > 0) {
                    ...
                    if (getSession()->getProjectIt()->getMethodologyIt()->get('IsSupportUsed') === 'Y') {
                        $this->addInfoSection(new DocumentationSection($object_it));
                    }
                }
            }
		}
    ...
    {
		return new RequestBulkForm($this->getObject(), $entityFormClass);
	}
