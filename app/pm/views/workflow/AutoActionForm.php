...
class AutoActionForm extends PMPageForm
{
    ...
    function getFieldDescription($field_name)
    {
        ...
        {
            case 'Recurring':
                $moduleIt = $module->getExact('dicts-recurring');
                return sprintf(text(3104), $moduleIt->getUrl());

            case 'WebhookPayload':
                return sprintf(text('autoaction.webhook'),
                            $module->getExact('apidocs-list')->getUrl('search=request'),
                            $module->getExact('apidocs-list')->getUrl('search=user'),
                            $module->getExact('apidocs-list')->getUrl('search=comment')
                        );
        }
