...
class PMWikiDocument extends PMWikiTable
{
	...
	protected function buildDocumentIt()
	{
        if ( !in_array($_REQUEST['page'], array('', 'all')) )
        {
            ...
            $documentIt = $this->getObject()->getRegistryBase()->Query(
                    array(
                        new FilterInPredicate($pageIt->get('DocumentId'))
                    )
                );
    ...
    function getLeftPaneTemplate() {
        return 'pm/WikiDocumentTree.php';
    }
    ...
    function getLeftPaneVisible() {
        return in_array($_COOKIE['toggle-docstruct'], array('','true'))
            && array_sum($this->getDocumentIt()->fieldToArray('TotalCount')) > 0;
    }
    ...
    function getCompareToActions()
	{
        ...
        $registry = new ObjectRegistrySQL($this->getObject());
        if ( !\TextUtils::isNullValue($this->getDocumentIt()->get('UID')) )
        {
            ...
            $branchesIt = $this->getObject()->getEmptyIterator();
        }
        ...
        if ( !\TextUtils::isNullValue($this->getDocumentIt()->get('UID')) )
        {
            ...
            $baselineIt = $this->getBaselineObject()->getEmptyIterator();
        }
 	...
 	function getRenderParms( $parms )
 	{
        ...
        if ( $_REQUEST['comparemode'] == 'modified' ) {
            array_unshift(
                $filter_settings,
                array(
                    'html' => '<a class="btn btn-sm btn-light" href="'.$this->getDocumentIt()->getUidUrl().'">&#8592; '.translate('Закрыть').'</a>'
                )
            );
        }
        ...
        return array_merge( $parent_parms, array (
            ...
            'placeholderClass' => 'wiki-page-document',
	...
	function getCaption()
	{
		return $this->getDocumentIt()->getDisplayNameSearch();
	}
    ...
    protected function buildQuickReports(& $base_actions)
    {
        $items      = [
            'projects' => [
                ...
                'items' => [
                    [
                        'name' => text('doc.open.registry'),
                        'url' => $this->getListViewWidgetIt()->getUrl('group=none&type=all&document='.$this->getDocumentIt()->getId()),
                        'uid' => 'registry'
                    ]
                ]
            ]
