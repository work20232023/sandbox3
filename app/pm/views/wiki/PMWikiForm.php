...
class PMWikiForm extends PMPageForm
{
    ...
    {
        return parent::getTransitionActions($this->getSectionIt());
    }
        ...
        while( !$converter_it->end() )
        {
            $this->exportMethods[] = array(
                ...
                'url' => $method->url(
                            ...
                            $converter_it->get('EngineParameters'),
	...
	function persist()
    {
        ...
        if ( $this->getAction() == 'add' ) {
            ...
            if ( $objectIt->get('ParentPage') == '' && $objectIt->get('DocumentVersion') != '' )
            {
                ...
                while( !$it->end() ) {
                    if ( $it->getId() != '' && $it->getDisplayName() == $objectIt->get('DocumentVersion') ) {
                        $baselineIt = $it->copy();
                        break;
                    }
                    $it->moveNext();
	...
	function getExportActions( $object_it )
	{
        ...
        foreach( $this->exportMethods as $action )
        {
            ...
            $action['url'] = preg_replace(
                '/%baseline%/',
                is_object($this->getRevisionIt()) && $this->getRevisionIt()->getId() > 0
                                ? $this->getRevisionIt()->getId()
                                : '0',
                $action['url']);

            if ( $object_it->count() > 1 ) {
                $action['url'] = preg_replace('/[:]?ExportChildren|[:]?ExportParents/', '', $action['url']);
            }

            if ( $object_it->get('ParentPage') == '' ) {
                $action['url'] = preg_replace('/[:]?ExportParents/', '', $action['url']);
            }
