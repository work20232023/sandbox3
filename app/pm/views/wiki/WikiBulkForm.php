...
class WikiBulkForm extends BulkForm
{
	function getIt()
	{
        ...
        if ( !$this->object instanceof \WikiPage ) return $iterator;
		...
		if ( strpos($_REQUEST['operation'], 'BulkDeleteWebMethod') === false )
        {
            if ( $this->getBaseline() != '' )
            {
                $version_it = getFactory()->getObject('Snapshot')->getExact($this->getBaseline());
                if ( $version_it->getId() != '' ) {
                    $registry = new WikiPageRegistryVersion($registry->getObject());
                    $registry->setDocumentIt($iterator->getRef('DocumentId'));
                    $registry->setSnapshotIt($version_it);
                }
            }

            return $registry->useImportantPersistersOnly()->Query(
                array (
    ...
    {
        $matches = [];
        if ( preg_match('/compare=([^:=]+)/', $_REQUEST['operation'], $matches) ) {
    ...
    {
        parent::drawIds( $value );

        echo '<input type="hidden" name="baseline" value="'.$this->getBaseline().'">';
        echo '<input type="hidden" name="compare" value="'.$this->getCompareTo().'">';
    }
}
