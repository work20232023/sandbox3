...
class PMWikiList extends PMPageList
{
    ...
    function extendModel()
    {
        ...
        $this->aggregateObject = getFactory()->getObject(get_class($this->getObject()));
        $this->aggregateObject->setRegistry(new ObjectRegistrySQL());
        $this->aggregateObject->setPersisters([]);
	...
	{
        $selfAttributes = ['State', 'Readiness'];

        if ( $object_it->get('Includes') > 0 && !in_array($attr, $selfAttributes) )
        {
            $includesIt = $object_it->object->getExact($object_it->get('Includes'));
            if ( $includesIt->get('Includes') == '' ) {
                return $this->drawCell( $includesIt, $attr );
            }
        }
		...
		switch ( $attr )
		{
			case 'Caption':
				...
				if ( $object_it->get('Suspected') > 0 ) {
                    $title = WidgetService::getHtmlBrokenIcon($object_it->getId(), getSession()->getApplicationUrl()) . $title;
				}
				...
				if ( $this->displayContent )
                {
                    ...
                    if ( $object_it->getId() > 0 && $compareToIt->getId() > 0 )
                    {
            ...
            case 'Readiness':
                echo '<div class="state-rich">';
                    ...
                    $hiePredicate = $object_it->get('ParentPage') == ''
                            ? new WikiDocumentFilter($object_it)
                            : new ParentTransitiveFilter($object_it->getId());

                    $agg_it = $this->aggregateObject->getAggregated(
                                [$aggregateFunc],
                                [$hiePredicate]
                            );
