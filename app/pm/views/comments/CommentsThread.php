...
class CommentsThread
{
    ...
    private $autorefresh = true;
 	...
 	function __construct( $object_it, $commentObject, $baseline = '', $uid = '' )
 	{
        ...
        $this->options = new CommentNotificationService();

        $this->systemName = getFactory()->getObject('SystemSettings')->getAll()->getDisplayName();
        $this->newCommentWebMethod = new CommentWebMethod($this->object_it);
 	}
 	...
 	function getRenderParms()
	{
		...
		return [
            ...
            'public_comment' => true,
            'private_comment' => count($this->options->getEmails($this->object_it)) > 0,
            'options' => $this->getListOptions(),
            'optionsDefault' => array('order', 'open'),
            'readonly' => !$this->newCommentWebMethod->hasAccess(),
		];
	}
	...
	{
	    $readonly = !$this->newCommentWebMethod->hasAccess();
	    $privateEnabled = $comment_it->get('IsPrivate') != 'Y'
            && count($this->options->getEmails($this->object_it, $comment_it)) > 0;
