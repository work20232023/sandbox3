...
class PMPageForm extends PageForm
{
	...
	protected function buildCustomAttributes()
	{
        ...
        while (!$it->end())
        {
            ...
            if ($it->get('ObjectKind') != '') {
                ...
                if ( !$this->getEditMode() ) {
                    if ( $it->get('ObjectKind') != $this->getDiscriminator() ) {
                        $this->getObject()->setAttributeVisible($it->get('ReferenceName'), false);
                    }
                }
            }
    ...
    function persist()
    {
        ...
        $transitionIt = $this->getTransitionIt();
        if ( $transitionIt->getId() != '' ) {
            $_REQUEST['State'] = $transitionIt->getRef('TargetState')->get('ReferenceName');
        }
    ...
    function IsAttributeVisible( $attr )
    {
 	...
 	function IsAttributeRequired($attr)
 	{
 		if (array_key_exists($attr, $this->customkinds)) {
 			...
 			if ($discriminator != $this->customkinds[$attr]) {
                return false;
            }
 		}
    ...
    function createFieldObject($attr)
    {
        switch ($attr) 
        {
            ...
            case 'TransitionNotification':
                ...
                if ( is_object($this->getObjectIt()) ) {
                    $options = new CommentNotificationService();
                    $field->setEmails($options->getEmails($this->getObjectIt()));
            ...
            default:
                ...
                if (in_array(PMCustomAttribute::KEY_DICTIONARY, $attributeGroups)) {
                ...
                if ( in_array('clipboard', $attributeGroups) ) {
                    return new FieldCopyClipboard();
                }

                if (in_array(PMCustomAttribute::KEY_COMPUTED, $attributeGroups) && is_object($this->getObjectIt())) {
                    return new FieldComputed($this->getObjectIt(), $attr);
                }

                if (in_array(PMCustomAttribute::KEY_CHECKLIST, $attributeGroups)) {
                    $object     = $this->getObjectIt();
                    $editMode   = $this->getEditMode();

                    if (null === $this->getObjectIt()) {
                        $object = $this->getObject()->getEmptyIterator();
                    }

                    return new FieldCheckList($object, $attr, $editMode);
                }
                ...
                if ($this->getObject()->getAttributeType($attr) == 'wysiwyg') {
                    ...
                    (is_object($object_it)) ? $field->setObjectIt($object_it) : $field->setObject($this->getObject());
    ...
    {
        return array_intersect(array_keys($this->getObject()->getAttributes()), ['State', 'Project']);
    }
	...
	function drawScripts()
	{
    	...
    	<script type="text/javascript">
            completeUICustomFields(
                ...
                <?=json_encode(array_keys($this->customkinds))?>,
                <?=json_encode(array_values($this->customkinds))?>
            );
    	</script>
    	<?php
	    }
    ...
    function getHintId()
    {
        if (!empty($this->getTransitionIt()->getId())) {
    ...
    {
        if (!empty(trim($this->getTransitionIt()->getId()))) {
