...
class PMPageList extends PageList
{
	...
	function drawCell( $object_it, $attr )
    {
        switch ( $attr )
        {
            ...
            default:
                $attributeGroups = $object_it->object->getAttributeGroups($attr);

                if ( in_array('computed', $attributeGroups) )
                {
                    if ( $object_it->object->getAttributeType($attr) == 'float' && is_numeric($object_it->get($attr)) ) {
                        parent::drawCell( $object_it, $attr );
                        break;
                    }
                ...
                if ( in_array('checklist', $attributeGroups) )
                {
                    $items = array_map(
                        function($item) {
                            $title = $item['name'];
                        ...
                        array_filter( \JsonWrapper::decode($object_it->getHtmlDecoded($attr)),
                            function($item) {
                                return $item['name'] != '';
                            }
                        )
    ...
    function getGroupEntityName( $groupField, $object_it, $referenceIt )
    {
        ...
        if (
            $referenceIt->object instanceof Request ||
            $referenceIt->object instanceof User
            )
            { 
            return "";
        }
