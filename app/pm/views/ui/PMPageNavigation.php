...
class PMPageNavigation extends PageNavigation
{
    ...
    function getMenus()
    {
        ...
        $actions['profile-my-tasks'] = array (
            'name' => text(3141),
            'url' => $report->getExact('mytasks')->getUrl(),
            'uid' => 'profile-my-tasks'
        );

        $reportIt = $report->getExact('activity');
        $actions['profile-my-activity'] = array (
            'name' => $reportIt->getDisplayName(),
            'url' => $reportIt->getUrl(),
            'uid' => 'profile-my-activity'
        );

        $actions['profile-my-reports'] = array (
            'name' => text(1811),
            'url' => $module->getExact('project-reports')->getUrl(),
    ...
    function getHelpActions()
    {
        ...
        return array_merge(
            array(
                ...
                array (
                    'name' => text(3400),
                    'url' => $module->getExact('apidocs-list')->getUrl(),
                    'uid' => 'dev-guide',
                    'target' => '_blank'
                )
            ),
