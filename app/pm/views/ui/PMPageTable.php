...
class PMPageTable extends PageTable
{
    ...
    function getCaption()
    {
        $title = $this->getWidgetIt()->getDisplayName();
 		...
 		if (!in_array($_REQUEST['baseline'], ['', 'none', 'all'])) {
	...
	{
		if (is_object($this->widget_it)) {
            return $this->widget_it;
        }
	...
	protected function buildWidgetIt()
	{
		...
		if ($this->getReport() == '') {
			...
			if ($this->getPage()->getModule() == '') {
                return $module->getEmptyIterator();
            }
    ...
    function getFilterMoreActions()
    {
        ...
        if (count($save_actions) > 0) {
            $actions = array_merge($actions, [[]], $save_actions);
        }
	...
	function getExportActions()
	{
		...
		$actions[] = [
			...
			'name' => text(2202),
			'url' => $method->url( $this->getCaption(), 'IteratorExportExcel', ['show' => 'all'] )
        ];
	...
	function getFiltersUrl()
	{
	    ...
	    foreach([$idKey, 'ids', 'offset1'] as $parmKey) {
	        if ( $_REQUEST[$parmKey] != '' ) {
	...
	function getSaveActions($actions)
    {
        ...
        if (!getFactory()->getAccessPolicy()->can_create($custom)) {
            return $actions;
        }
    ...
    protected function buildFamilyFilterItems()
    {
        ...
        $selfIds = [$this->getReport(), $this->getReportBase()];
        $self_it = $report->getExact($selfIds);
        $modules = array_merge([$this->getPage()->getModule()], $familyModules);
    ...
    protected function buildChartFilterItems()
    {
        ...
        if ($chartsModule != '') {
            $moreReports[] = [
                'name' => text(2628),
                'url' => $module->getExact($chartsModule)->getUrl($urlParmsString)
            ];
        }
	...
	protected function buildQuickReports(& $base_actions)
	{
        ...
        if (!$this instanceof DashboardTable && !$this instanceof PMWikiDocument && $widgetIt->getId() != '') {
        ...
        if (count($items) > 0) {
            $base_actions = array_merge($items, [[]], $base_actions);
        }
	}
	...
	function buildOpenInOtherMenu()
    {
        ...
        while ( !$linked_it->end() ) {
            $parentActions[$linked_it->get('CodeName')] = [
        ...
        while ( !$project_it->end() ) {
            $parentActions[$project_it->get('CodeName')] = [
        ...
        while ( !$portfolio_it->end() ) {
            ...
            if ( in_array($selfIt->getId(), $project_ids) || in_array($portfolio_it->get('CodeName'), ['my','all']) ) {
    ...
    function buildCustomPredicates( $values )
	{
        ...
        while( !$attr_it->end() ) {
            ...
            if ( in_array($type, ['dictionary','reference','char']) ) {
	...
	public function buildFilterValuesByDefault( & $filters )
	{
        ...
        foreach( $this->filters as $filter ) {
            if ( !$filter instanceof WebMethod ) {
                continue;
            }
	...
	public function buildFilterValuesBySettings( $default_values, & $setting )
	{
		$values = [];
	 	...
	 	if ( is_object($setting) ) {
	    	...
	    	if ( $setting->getRowsNumber() != '' ) {
                $values['rows'] = $setting->getRowsNumber();
            }
	 	}
		...
		$alt_setting = $setting instanceof ReportSetting 
 	    ...
 	    if ( is_object($alt_setting) ) {
 	    	if ( $alt_setting->getGroup() != '' ) {
                $values['group'] = $alt_setting->getGroup();
            }
            ...
            $sections = $alt_setting->getSections();
 	    	...
 	    	if ( count($sections) > 0 ) {
                $values['infosections'] = join(',',$sections);
            }
 	    }
    ...
    {
        return new FilterTraceWebMethod($this->getObject());
    }
    ...
    {
        $resolvedAmount = (new \ObjectRegistrySQL($this->getObject()))->Count([
            new FilterVpdPredicate(), new StatePredicate('terminal')
        ]);
	...
	function getSortFields()
	{
	    ...
	    if (in_array('State', $system_attributes)) {
            unset( $system_attributes[array_search('State', $system_attributes)] );
        }
    ...
    function getFullPageRenderParms( $parms )
	{
		...
		return array_merge($parms, [
            ...
            'details' => $this->getPage()->getReportChart() ? [] : $this->getDetails(),
    ...
    function getDetails()
    {
        return [
            ...
            'network' => [
                'image' => 'icon-random',
                'title' => text(2167),
                'url' => getSession()->getApplicationUrl().'graph/%class%/%id%'
            ],
            'props' => [
                'image' => 'icon-zoom-in',
                'title' => text(2167),
    ...
    {
        if (count(\TextUtils::parseIds($filterValues['target'])) == 1) {
            return true;
        }
        ...
        foreach($values as $attributes) {
            if (count(array_diff($example, $attributes)) > 0 || count(array_diff($attributes, $example)) > 0) {
                return false;
            }
        }
