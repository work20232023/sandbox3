...
{
 	var $object_it, $caution, $anchor_field;
	private $object = null;
	...
	private $appendButtonTemplate = 'pm/AttachmentsAppendButton.php';
 	...
 	function getAttachmentIt()
 	{
 		if ( is_object($this->object_it) ) {
            return getFactory()->getObject('pm_Attachment')->getRegistry()->Query(
                array(
                    new AttachmentObjectPredicate($this->object_it)
                )
            );
 		}
 		else {
 	...
 	function render( $view )
	{
        ...
        if ( !getFactory()->getAccessPolicy()->can_create($attachmentIt->object) ) return;
