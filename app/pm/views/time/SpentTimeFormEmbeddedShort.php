...
class SpentTimeFormEmbeddedShort extends SpentTimeFormEmbedded
{
    ...
    function getFieldValue($attr)
    {
        switch( $attr ) {
            case 'LeftWork':
                $anchorIt = $this->getAnchorIt();
                if ( is_object($anchorIt) ) {
                    return $anchorIt->get($this->getObject()->getParentLeftWorkName());
                }
                break;
        }
	...
	{ 
        ...
        if ( $this->showAutoTimeButtons && getFactory()->getAccessPolicy()->can_create($this->getObject()) ) {
