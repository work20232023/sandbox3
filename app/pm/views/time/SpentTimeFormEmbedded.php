...
class SpentTimeFormEmbedded extends PMFormEmbedded
{
    ...
    function drawAutoTimes( $submit )
    {
        ...
        if ( $objectIt->get('StateDurationRecent') > 0 ) {
            $workHours = strval(
                            defined('MAX_DAILY_HOURS')
                            ? min(MAX_DAILY_HOURS, $objectIt->get('StateDurationRecent'))
                            : $objectIt->get('StateDurationRecent')
                         );
    ...
    function getListUrl()
    {
        $attribute = $this->getAnchorField();
        $object = $this->getIteratorRef()->object;
        $ids = $this->getIteratorRef()->fieldToArray($attribute);

        return WidgetUrlBuilder::Instance()
                    ->buildWidgetUrlIds(get_class($object), $ids, strtolower($attribute));
    }
}
