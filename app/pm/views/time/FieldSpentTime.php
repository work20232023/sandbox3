...
class FieldSpentTime extends FieldForm
{
	...
	function drawBody( $view = null )
	{
		...
		$activity->addSort( new SortAttributeClause('ReportDate') );
 		...
 		$form->showAutoTimeButtons( !$this->short_form && !$this->readOnly() );
        ...
        if ( $this->showTotal && is_object($object_it) && $object_it->get('Fact') > 0 ) {
            echo '<div>&nbsp;</div><div><span title="'.$object_it->object->getAttributeDescription('Fact').'">'.
                    sprintf(text('time.total'), getSession()->getLanguage()->getHoursWording($object_it->get('Fact'))).'</span></div>';
        }
