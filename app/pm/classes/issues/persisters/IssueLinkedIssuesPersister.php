 	...
 	function getSelectColumns( $alias )
 	{
        ...
        if ( getSession()->IsRDD() )
        {
            $columns[] =
                " (SELECT GROUP_CONCAT(CAST(l.TargetRequest AS CHAR)) 
			        FROM pm_ChangeRequestLink l,
			             pm_ChangeRequest tar 
					WHERE l.SourceRequest = {$this->getPK($alias)}
					  AND l.TargetRequest = tar.pm_ChangeRequestId
					  AND tar.Type IS NOT NULL ) DownstreamLinks ";
        }
        else {
            $columns[] =
                " (SELECT GROUP_CONCAT(CAST(l.TargetRequest AS CHAR)) 
			        FROM pm_ChangeRequestLink l 
					WHERE l.SourceRequest = {$this->getPK($alias)}) DownstreamLinks ";
        }
