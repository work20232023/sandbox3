...
class RequestMetricsEventHandler extends SystemTriggersBase
{
	...
	{
        if ( $object_it->object instanceof RequestTraceMilestone && $object_it->get('ChangeRequest') != '' )
        {
            $kind = TRIGGER_ACTION_MODIFY;
            getFactory()->resetCache();

            $object_it = getFactory()->getObject('Request')->getRegistryBase()->Query(
                    array(
                        new FilterInPredicate($object_it->get('ChangeRequest')),
                        new RequestMilestonesPersister()
                    )
                );

            // force update Deadlines that triggers sync attributes of a link
            $object_it->object->modify_parms( $object_it->getId(),
                array(
                    'Deadlines' => $object_it->get('Deadlines')
                )
            );
        }
