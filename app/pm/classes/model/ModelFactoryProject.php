...
include "validators/ModelValidatorWorkAssignedLimit.php";
include "validators/ModelValidatorScript.php";
...
class ModelFactoryProject extends ModelFactoryExtended
{
	...
	function getModelValidators()
    {
        return array_merge(
            ...
            [
                ...
                new ModelValidatorWorkAssignedLimit(),
                new ModelValidatorScript()
            ]
        );
    }
    ...
    {
        $lock = new LockFileSystem('project-cache');
        $lock->Wait(PAGE_WAIT_SECONDS);
        $lock->Lock();
    ...
    public function modifyEntity( $objectIt, $parms, $validators = array(), $mappers = array(), $disableNotifications = false )
    {
        ...
        return parent::modifyEntity($objectIt, $parms, $validators, $mappers, $disableNotifications);
    }
