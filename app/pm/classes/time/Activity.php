...
include "predicates/ActivityIssuePredicate.php";
include "validators/ModelValidatorDayTimeSpentLimit.php";
include "validators/ModelValidatorPlannedTimeSpentLimit.php";
...
class Activity extends Metaobject
{
 	function __construct( ObjectRegistry $registry = null )
 	{
        ...
        $this->setAttributeRequired('Participant', true);
    ...
    function getParentLeftWorkName() {
         return 'LeftWork';
    }
	...
	function delete( $id, $record_version = ''  )
	{
		...
		if ( $activity_it->get('Task') > 0 )
		{
		    ...
		    $it = getFactory()->getObject('Activity')->getAggregated( [$sum_aggregate], [
                new FilterAttributePredicate('Task', $task_it->getId())
            ]);
    ...
    function getValidators()
    {
        return array_merge(
            ...
            array(
                new ModelValidatorDayTimeSpentLimit(),
                new ModelValidatorPlannedTimeSpentLimit()
            )
        );
    }
}