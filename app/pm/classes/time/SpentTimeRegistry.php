 	...
 	function Query( $parms = array() )
 	{
        ...
        foreach( $filters as $filter ) {
            if ( $filter instanceof FilterSubmittedAfterPredicate ) {
                $value = $mapping->map(array_shift(\TextUtils::parseFilterItems($filter->getValue())));
		...
		$sql = " SELECT {$registry->getSelectClause(array(), 't')},
			       ...
			       FROM (SELECT t.ChangeRequest,
                                ...
                                a.Description,
                                a.Issue,
			   				    IFNULL(r.VPD, t.VPD) VPD, 
			   				    a.Participant cms_UserId, 
			   				    a.Participant {$userField}, 
			   				    {$participantField}
			   				    a.pm_ActivityId OrderNum,
			    				(SELECT p.pm_ProjectId FROM pm_Project p WHERE p.VPD = IFNULL(r.VPD, t.VPD)) Project, 
			    				t.State, 
			                    u.Rate
			      ...
			      ORDER BY {$registry->getSortClause($this->extractSorts($parms), 't')} ";
