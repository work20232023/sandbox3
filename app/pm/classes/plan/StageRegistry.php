 	...
 	function getQueryClause(array $parms)
 	{
 		...
 		$sql = " SELECT 
                    ...
                    (SELECT SUM(a.Capacity) FROM pm_Activity a, pm_ChangeRequest s 
                      WHERE s.PlannedRelease = v.pm_VersionId AND a.Issue = s.pm_ChangeRequestId AND a.IsApproved = 'Y') IssuesFact, 
                    (SELECT SUM(a.Capacity) FROM pm_Activity a, pm_Task s, pm_ChangeRequest r 
                      WHERE r.PlannedRelease = v.pm_VersionId AND r.pm_ChangeRequestId = s.ChangeRequest AND a.Task = s.pm_TaskId AND a.IsApproved = 'Y') TasksFact, 
                    'Release' State, 
        ...
        $iteration = getFactory()->getObject('Iteration');
 		if ( getSession()->getProjectIt()->getMethodologyIt()->HasPlanning() && getFactory()->getAccessPolicy()->can_read($iteration) )
        {
            $sql .=
                " UNION 
		     SELECT IF(v.Caption IS NULL, LPAD(r.Caption,8,'0'), concat(LPAD(v.Caption,8,'0'), '.', LPAD(r.Caption,8,'0'))), 
                ...
                ($projectPrefix ? "CONCAT('{',(SELECT p.Caption FROM pm_Project p WHERE p.VPD = r.VPD),'} ')" : "''"). " CaptionPrefix, 
                    IF(v.Caption IS NULL, CONCAT('{$iteration->getDisplayName()} ',r.Caption), CONCAT('{$iteration->getDisplayName()} ',v.Caption,'.',r.Caption)), 
                    r.pm_ReleaseId, 
                    v.pm_VersionId, 
                    ...
                    (SELECT SUM(a.Capacity) FROM pm_Activity a, pm_ChangeRequest s 
                      WHERE s.Iteration = r.pm_ReleaseId AND a.Issue = s.pm_ChangeRequestId AND a.IsApproved = 'Y') IssuesFact, 
                    (SELECT SUM(a.Capacity) FROM pm_Activity a, pm_Task s 
                      WHERE s.Release = r.pm_ReleaseId AND a.Task = s.pm_TaskId AND a.IsApproved = 'Y' ) TasksFact, 
                    'Iteration' State, 
                    0 ChildrenCount,
                    r.IsClosed
               FROM (SELECT r.*, 
        ...
        if ( getFactory()->getAccessPolicy()->can_read($request) )
        {
            ...
            $sql .=
		     ...
		     SELECT '00000001', 
                    ...
                    '' RecentComment,
                    '', 
                    ".getSession()->getProjectIt()->getId().", 
                    '{$request->getVpdValue()}', 
                    NOW(), 
                    NOW(), 
                    (SELECT GROUP_CONCAT(CAST(r.pm_ChangeRequestId AS CHAR))
                       FROM pm_ChangeRequest r 
                      WHERE r.PlannedRelease IS NULL 
                        AND r.Iteration IS NULL 
                        ...
                        AND NOT EXISTS (SELECT 1 FROM pm_ChangeRequestLink a, pm_ChangeRequestLinkType lkt
                                         WHERE a.TargetRequest = r.pm_ChangeRequestId
                                           AND a.LinkType = lkt.pm_ChangeRequestLinkTypeId
                                           AND lkt.ReferenceName IN ('implemented') )
                        {$request->getVpdPredicate('r')} ), 
                    NULL,
                    1 UncompletedIssues, 
                    (SELECT GROUP_CONCAT(CAST(s.pm_TaskId AS CHAR)) 
                       FROM pm_Task s 
                      WHERE s.Release IS NULL 
                        AND s.ChangeRequest IS NULL {$task->getVpdPredicate('s')}) Tasks, 
                    1 UncompletedTasks, 
                    NULL, 
                    NOW(),
                    NOW(), 
