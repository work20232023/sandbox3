...
class WikiPage extends MetaobjectStatable
{
	...
	function add_parms( $parms )
	{
        if ( $parms['PageType'] != '' && is_numeric($parms['PageType']) ) {
            $parms['IDType'] = $this->increaseCounter($parms['PageType'], true);
        }

        $parms['IDProject'] = $this->createSQLIterator(
                                    " SELECT COUNT(1) + 1 Counter FROM WikiPage t 
                                           WHERE t.VPD = '{$this->getVpdValue()}'
                                             AND t.ReferenceName = {$this->getReferenceName()}
                                             AND t.IsTemplate = 0 ")->get('Counter');
	...
	function modify_parms( $id, $parms )
	{
        ...
        $parms['Modifier'] = getSession()->getUserIt()->getId();
	...
	public function updateUIDs( $objectIt )
    {
        ...
        while( !$objectIt->end() )
        {
            if ( $objectIt->get('MasterPageUID') != '' ) {
                DAL::Instance()->Query(
                    "UPDATE WikiPage w SET w.UID = '{$objectIt->get('MasterPageUID')}' WHERE w.WikiPageId = {$objectIt->getId()}"
                );
