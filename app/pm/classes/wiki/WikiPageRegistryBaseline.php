...
class WikiPageRegistryBaseline extends ObjectRegistrySQL
{
	...
	function getQueryClause(array $parms)
	{
	    ...
	    foreach( $this->getObject()->getAttributes() as $attribute => $data ) {
	    	if ( in_array($attribute, array('Caption', 'Content', 'DocumentVersion', 'DataHash')) ) continue;
        ...
        $attributesString = join(",",$attributes);

	    return " (SELECT t.WikiPageId, t.VPD, t.RecordVersion, t.DocumentId, t.DocumentVersion, t.SortIndex, t.Caption, t.Content, t.DataHash, {$attributesString}
	    	   	    FROM WikiPage t
	    	       WHERE t.DocumentId = {$this->document_it->getId()} {$sqlPredicate}
	    	       UNION 
	    	      SELECT t.WikiPageId, t.VPD, NULL, {$this->document_it->getId()}, '{$this->document_it->getHtmlDecoded('DocumentVersion')}', t.SortIndex, NULL, NULL, NULL, {$attributesString}
	    	   	    FROM WikiPage t 
