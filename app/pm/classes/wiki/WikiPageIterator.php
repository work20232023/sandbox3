...
class WikiPageIterator extends StatableIterator
{
        ...
        if ( $baselineId > 0 ) {
            ...
            if ( $baselineIt->getId() != '' ) {
                $this->setData(
                    array_merge(
                        $this->getData(),
                        ['DocumentVersion' => $baselineIt->getDisplayName()]
                    )
                );
            }
        }
	...
	function getParentsIt( $object = null )
	{
		...
		return $object->getRegistryBase()->Query( array(
            ...
            new FilterNotInPredicate($this->getId())
		));
	}
    ...
    function getHash()
    {
        return $this->get('IncludesDataHash') != '' ? $this->get('IncludesDataHash') : $this->get('DataHash');
    }
