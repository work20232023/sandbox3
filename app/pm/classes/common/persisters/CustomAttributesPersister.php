...
class CustomAttributesPersister extends ObjectSQLPersister
{
    ...
    private $attributeRowset = null;

    function __construct(array $attributes, array $attributeRowset)
    {
        $this->attributeRowset = array_values($attributeRowset);
        parent::__construct($attributes);
    }

    protected function getAttributeIt()
    {
        return (new \PMCustomAttribute())->createCachedIterator($this->attributeRowset);
    }
    ...
    function IsPersisterImportant()
    {
 	...
 	protected function getAttributesInfo()
 	{
        ...
        $attributes = array();
        $lastAttributes = array();
        $attr_it = $this->getAttributeIt();
 		while ( !$attr_it->end() )
		{
            $typeIt = $attr_it->getRef('AttributeType');
 			$item = array (
			    'id' => $attr_it->getId(),
            ...
            $typeIt->get('ReferenceName') == 'computed'
                ? $lastAttributes[] = $item
                : $attributes[] = $item;
 		...
 		return $this->attrs = array_merge($attributes, $lastAttributes);
 	}
    ...
    function modify( $object_id, $parms ) {
        $this->set($object_id, $parms, false, false);
    }
	...
	public function __sleep() {
		return array_merge(
			parent::__sleep(), array('attrs', 'references', 'attributeRowset')
		);
	}
