	...
	function getSelectColumns( $alias )
 	{
        ...
        $columns[] = "( SELECT UNIX_TIMESTAMP(NOW()) / 3600 - 
 			                   IFNULL( (SELECT UNIX_TIMESTAMP(self.RecordCreated) / 3600 - IFNULL(SUM(so.Duration),0) 
                                         ...
                                         WHERE self.pm_StateObjectId = {$alias}.StateObject 
                                           ...
                                           AND self.ObjectClass = so.ObjectClass), 
 			                            UNIX_TIMESTAMP({$alias}.RecordCreated)/ 3600 
 			                            ) 
 			                   ) StateDuration ";
