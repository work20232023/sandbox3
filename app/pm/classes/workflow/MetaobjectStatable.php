...
include "persisters/ArtefactUserReviewItemPersister.php";
...
class MetaobjectStatable extends Metaobject 
{
	...
	function modify_parms( $object_id, $parms )
	{
        ...
        if ( $parms['State'] != '' && ($object_it->get('State') != $parms['State'] || $parms['Transition'] != '') )
		{
            $wasState = $parms['State'];
            if ( $parms['VPD'] != '' && $parms['VPD'] != $object_it->get('VPD') ) {
                $parms['State'] = $this->reMapState( $parms['VPD'], $parms['State'] );
            }
			if ( $wasState != '' && ($object_it->get('State') != $parms['State'] || $parms['Transition'] != '') ) {
	...
	function delete ( $id, $record_version = ''  )
	{
		...
		{
            DAL::Instance()->Query(
                " DELETE FROM pm_StateObject WHERE ObjectId = {$id} AND ObjectClass = '{$this->getStatableClassName()}' "
            );
		}
	...
	public function moveToState( $object_it, & $parms )
	{
		...
		$self_it = $registry->Query(
			array(
                ...
                new ArtefactUserReviewItemPersister(),
		...
		if ( $parms['TransitionComment'] != '' )
		{
            ...
            $comment_id = $commentIt->getId();
		}
        unset($parms['TransitionComment']);
        ...
        $parms['StateObject'] = $objectstate->add_parms(
            array (
                ...
                'ReviewItem' => $self_it->get('ArtefactUserReviewItem'),
