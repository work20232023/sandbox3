...
class AutoAction extends Metaobject
{
 	function __construct()
 	{
        ...
        foreach( array('EmailSubject','EmailBody','EmailRecipients','EmailAdditional') as $attribute ) {
            $this->addAttributeGroup($attribute, 'email');
        }
        $this->setAttributeType('EmailBody', 'WYSIWYG');
        $this->setAttributeType('EmailRecipients', 'REF_MentionedId');
        $this->addAttributeGroup('EmailRecipients', 'multiselect');
 	}
 	...
 	function getAttributeObject($attribute)
    {
        ...
        if ( $attribute == 'EmailRecipients' && $object instanceof \Mentioned ) {
            $object->setAttributesObject(getFactory()->getObject($this->getSubjectClassName()));
        }
