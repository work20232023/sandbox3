...
class ObjectMetadataStateAttributeBuilder extends ObjectMetadataEntityBuilder
{
    ...
    public function build( ObjectMetadata $metadata )
    {
        ...
        while( $this->stateAttributeIt->get('Entity') == $classFound )
        {
            ...
            if ( $this->stateAttributeIt->get('Groups') != '' )
            {
                ...
                $tabGroups = array_intersect($this->getTabGroups(),
                                \TextUtils::parseItems($this->stateAttributeIt->get('Groups')));
                if ( count($tabGroups) > 0 ) {
                    $groups = array_diff($groups, $this->getTabGroups());
                }
    ...
    {
        return ['trace','additional','deadlines','efforts'];
    }
}