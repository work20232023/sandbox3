...
class CloneLogic
{
			...
			switch ( $object->getEntityRefName() )
			{
				case 'pm_CustomReport':
					$ids_map[$object->getEntityRefName()][$iterator->getId()] =
                        self::applyToCustomReport( $context, $attrs, $object, $iterator, $project_it );
					break;
                ...
                case 'pm_DashboardItem':
                    ...
                    if ( $id > 0 ) {
                        ...
                        if ( count($parms) > 0 ) {
                            $ids_map[$object->getEntityRefName()][$iterator->getId()] = self::duplicate( $object, $iterator, $context, $parms );
                        }
                    }
                    break;
				...
				case 'WikiPage':
					if( $iterator->object instanceof ProjectPage && $iterator->get('ParentPage') == '' ) {
						...
						if ( $kbrootId < 1 ) {
							$kbrootId = self::duplicate($object, $iterator, $context,
 	...
 	static function applyToObject( & $context, $attrs, & $parms, & $it, & $project_it )
 	{
		...
		switch ( $it->object->getEntityRefName() )
		{
			...
			case 'WikiPage':
                ...
                if ( $it->get('Includes') > 0 ) {
                    $mappedId = $ids_map[$it->object->getEntityRefName()][$it->get('Includes')];
                    if ( $mappedId == '' ) $mappedId = $it->get('Includes');

                    $mappedIt = $it->object->getExact($mappedId);
                    $parms['Content'] = $mappedIt->getHtmlDecoded('Content');
                    $parms['UID'] = $mappedIt->get('UID');
                    $parms['Includes'] = '';
                    $parms['Dependency'] = '';
                }
            ...
            case 'pm_Task':
                ...
                $parms['StartDate'] = '';
                $parms['DueWeeks'] = '';
                $parms['EstimatedStartDate'] = '';
                $parms['EstimatedFinishDate'] = '';

                $states = \WorkflowScheme::Instance()->getTerminalStates($it->object);
                if ( in_array($it->get('State'),$states) && $context->getRestoreFromTemplate() ) {
            ...
            case 'pm_Activity':
                ...
                if ( defined('MAX_DAILY_HOURS') ) {
                    $parms['Capacity'] = min($it->get('Capacity'), MAX_DAILY_HOURS);
                }
                break;
	...
	static function applyToCustomReport( $context, & $parms, $object, $it, $project_it )
	{
        $parms['IsPublic'] = 'Y';
        ...
        if ( $id < 1 ) {
            ...
            if ( count($parms) > 0 ) {
                return self::duplicate($object, $it, $context, $parms);
            }
        }
        else {
	...
	static function duplicate( $object, $iterator, $context, $parms )
	{
		$attributes =
            array_merge(
		...
		if ( $parms[$id_attribute] > 0 ) {
			...
			$temp_it = $object->getExact($parms[$id_attribute]);
		...
		if ( $object instanceof Request ) {
            $iterator = $object->createCachedIterator(array($values))->getSpecifiedIt();
            $object = $iterator->object;
        }

		try {
            $parms = array();
            foreach( $object->getAttributesByGroup('alternative-key') as $attribute ) {
                if ( $values[$attribute] == '' ) {
                    $parms = array();
                    break;
                }
            ...
            if ( count($parms) > 0 ) {
                ...
                $foundIt = $object->getRegistry()->Query($parms);
                if ( $foundIt->getId() != '' ) {
            ...
            $createdIt = getFactory()->createEntity($object, $values);
